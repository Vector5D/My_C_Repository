...
分治策略的根本特点是将一个规模比较大的问题，分解成许多规模较小的相同问题，问题不变，规模变小。递归一个函数直接或者间接地调用自己，称这个函数为递归函数。具体分为“递推”和“回归”两个步骤，由终止条件控制，即逐层递推，然后递归终止条件满足，终止递推，逐层回归。分治和递归是两个孪生兄弟，经常同时出现在一个算法设计当中，产生许多高效的代码。
...
```c
char c = fgetc(stdin);//从流中读取一个字符
char c = getc(stdin);//从流中读取一个字符
char c = getchar();//从标准输入流中读取一个字符

char str[10] = { 0 };
fgets(str, sizeof(str), stdin);//从流中读取长度为n-1的字符串，转储到str中
char str[10] = { 0 };
gets_s(str, sizeof(str));//从标准输入流中读取长度为n-1的字符串，转储到str中

fputc('c', stdout);//向流中写入一个字符
putc('c', stdout);//向流中写入一个字符

fputs("hello string",stdout);//向流中写入字符串
puts("hello string");//向流中写入字符串
putchar('c');//向标准输出流写入一个字符
```

总的来说，C语言处理字符与字符串时较为复杂。上面所有函数中，有以下几点需要注意：想要从流中读取一个字符，建议统一使用 fgetc 函数；想要从流中读取一个字符串，建议统一使用 fgets 函数；想要向流中写入一个字符，建议统一使用 fputc 函数；想要向流中写入一个字符串，建议统一使用 fputs 函数。
